...
async function main() {
  try {
    ...
    // Export MLELeague (must be first - it's referenced by MLETeam)
    console.log('Exporting MLELeague...');
    const mleLeagues = await prisma.mLELeague.findMany();
    writeFileSync(
      join(process.cwd(), 'exports', 'MLELeague.json'),
      JSON.stringify(mleLeagues, null, 2)
    );
    console.log(`✅ Exported ${mleLeagues.length} MLE Leagues`);
    ...
    console.log('\nFiles created:');
    console.log('  - exports/MLELeague.json');
